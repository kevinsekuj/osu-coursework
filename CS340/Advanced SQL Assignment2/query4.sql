...
JOIN film_actor ON actor.actor_id = film_actor.actor_id AND actor.actor_id NOT IN
    ...
    (SELECT actor.actor_id FROM actor
        ...
        JOIN film ON film_actor.film_id = film.film_id AND category.category_id = (SELECT category_id FROM category WHERE `name` = 'Animation') GROUP BY actor.actor_id) 
