...
LEFT JOIN film ON film_actor.film_id = film.film_id AND category.category_id = (SELECT category_id FROM category WHERE `name` = 'Animation') GROUP BY actor.actor_id ORDER BY actor.actor_id;