...
MonteCarlo( IN float *dtxs, IN float *dtys, IN float *dtxvs, IN float *dsvs, IN float *dsths, IN float *dhalflens, OUT int *dhits )
{
  ...
  float txv   = dtxvs[gid];
  float sv    = dsvs[gid];
  float svDegrees    = dsths[gid];
  float svRadians = Radians(svDegrees);
  float truckHalfLen = dhalflens[gid];

  float svx = sv * cos(svRadians);
  float svy = sv * sin(svRadians);
  ...
  float sbx = svx * t;
	...
	if (fabs(truckx - sbx) < truckHalfLen )
	{
		dhits[gid] = 1;
	}
}
...
main( int argc, char* argv[ ] )
{
  ...
  // fills in devices hit bit array
